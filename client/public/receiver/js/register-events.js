import { gamepadHandler } from "../../receiver/js/gamepadEvents.js";
...
export function sendClickEvent(videoPlayer, elementId) {
  ...
  console.log(InputEvent.ButtonClick);
  console.log(elementId);
  videoPlayer && videoPlayer.sendMsg(data.buffer);
}

export function sendMessage(videoPlayer, settings) {
  console.log("Aiii");
  let buffer = new ArrayBuffer(14); // One Int16 is 2 bytes, plus three Float32 (each 4 bytes) = 14 bytes.

  let view = new DataView(buffer);

  // Set Int16 value at the first index.
  view.setUint8(0, InputEvent.Message);

  // Set Float32 values.
  view.setFloat32(1, settings[0], true); // Offset is 1 byte.
  view.setFloat32(5, settings[1], true); // Offset is 5 bytes.
  view.setUint8(9, settings[2]);
  view.setFloat32(10, settings[3], true); // Offset is 9 bytes.

  // data.setFloat32(5, 0.5, true);
  videoPlayer && videoPlayer.sendMsg(view.buffer);
}
